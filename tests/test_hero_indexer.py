...
def test_hero_indexer_string_ids():
    indexer = HeroIndexer()

    # Heroes with string IDs and non-lexicographical integer order
    # String sort would be "1", "10", "2"
    # Integer numerical sort is 1, 2, 10
    heroes = [
        {"id": "10", "name": "Morphling", "playable": True},
        {"id": "2", "name": "Axe", "playable": True},
        {"id": "1", "name": "Anti-Mage", "playable": True},
    ]
    ...
    assert indexer.map_hero_id(1) == 1
    assert indexer.map_hero_id("1") == 1
    assert indexer.map_hero_id(2) == 2
    assert indexer.map_hero_id("2") == 2
    assert indexer.map_hero_id(10) == 3
    assert indexer.map_hero_id("10") == 3

    # Invalid non-integer hero IDs should raise ValueError instead of failing silently
    with pytest.raises(ValueError):
        indexer.map_hero_id("non_existent")

    with pytest.raises(TypeError):
        indexer.map_hero_id(None)
